#!/bin/bash
#
# Run Guaranteed Messaging fanout performance test:
#
# - creates subscribers and queues to receive fanned out messages, each queue subscribes to all topics
# - creates a single publisher that sends at rate x
# - calculates the total received message rate
# - setting subscriberConnections > 1 will run Load Balanced subscribers on non-exclusive queue
#
# SDKPerf params:
#	-msa = message size
...
do
	$SDK_PERF -cip="$SMF_URL" -cu=$MSG_USER@$VPN -cp=$MSG_PASS -stp=$topic -stc=$topicCount -sql=$queue$var -pe -pea 0 -cc $subscriberConnections >sub$var.out 2>sub$var.err &
	subPids+=($!)
done
...
$SDK_PERF -cip="$SMF_URL" -cu=$MSG_USER@$VPN -cp=$MSG_PASS -ptp=$topic -ptc=$topicCount -mt=persistent -msa=$messageSize -mn=$messageCount -mr=$publishRate -cc $publisherConnections>pub.out 2>pub.err &
pubPid=$!
